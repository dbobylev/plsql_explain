...
# Подробное описание с рекурсивной агрегацией блоков
...
Для больших методов (> 4000 символов) суммаризатор автоматически использует рекурсивный анализ по дереву операторов:

1. Загружает дерево операторов метода (IF, LOOP, SQL, EXCEPTION и т.д.) из результатов парсинга
2. Рекурсивно раскрывает большие блоки `BEGIN/IF/CASE/LOOP/EXCEPTION` до единиц анализа подходящего размера
3. Анализирует leaf-фрагменты, затем поднимается вверх и агрегирует summary веток, блоков и всего метода
4. Переиспользует промежуточные результаты из `analysis_cache`, если соответствующее поддерево не менялось
...
| `analysis_cache` | Кэш промежуточных анализов узлов дерева substatement для инкрементального пересчёта |
...
4. Для больших методов выполняется рекурсивный анализ по дереву операторов с агрегацией summary снизу вверх
