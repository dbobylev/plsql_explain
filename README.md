...
| `LLM_EMBEDDING_MODEL` | Модель эмбеддинга для RAG (через тот же `LLM_BASE_URL/v1/embeddings`), например `text-embedding-3-small` |
...
### Шаг 4 — Получить LLM-суммаризацию (и построить RAG-индекс)
...
### Шаг 5 — Семантический поиск (RAG)

После суммаризации можно проиндексировать узлы и делать семантический поиск по кодовой базе.

```bash
# Проиндексировать узлы описания для всей схемы
python main.py embed --schema MYSCHEMA

# Проиндексировать узлы конкретного пакета
python main.py embed --schema MYSCHEMA --object PKG_ORDERS

# Использовать конкретную embedding-модель
python main.py embed --schema MYSCHEMA --model text-embedding-3-small

# Семантический поиск
python main.py search "где обновляется статус заказа" --schema MYSCHEMA

# Ограничить результаты
python main.py search "обработка исключений при недостатке средств" --schema MYSCHEMA --top-k 5
```

Каждый узел индексируется как обогащённый текст: breadcrumb-путь в дереве + LLM-описание + обращения к таблицам + исходный код. Это позволяет находить нужные места даже без точного знания имён функций или таблиц.

Поиск работает через cosine similarity над эмбеддингами, хранящимися в SQLite (`node_embedding`). Для индексации и поиска используется тот же OpenAI-совместимый endpoint (`LLM_BASE_URL/v1/embeddings`).

#### Параметры `embed`

| Флаг | Описание |
|---|---|
| `--schema NAME` | Схема Oracle (обязательный) |
| `--object NAME` | Ограничить индексацию одним объектом |
| `--model NAME` | Переопределить модель эмбеддинга (иначе из `LLM_EMBEDDING_MODEL`) |
...
| `--schema NAME` | Ограничить поиск схемой |
| `--object NAME` | Ограничить поиск объектом |
| `--top-k N` | Количество результатов (по умолчанию: 10) |
| `--model NAME` | Переопределить модель эмбеддинга |
...
Oracle DBA_SOURCE
  ...
  [4] summarize    — иерархическая LLM-суммаризация снизу вверх
       ↓
  [5] embed        — эмбеддинг узлов описания в SQLite (RAG-индекс)
       ↓
  [5] search       — семантический поиск по описаниям (cosine similarity)
```
...
| `node_embedding` | Векторные эмбеддинги узлов для семантического поиска (RAG) |
...
| `rag/` | Семантический поиск: эмбеддинг узлов описания, хранение векторов в SQLite, cosine similarity поиск |
